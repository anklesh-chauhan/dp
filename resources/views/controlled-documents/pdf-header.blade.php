...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        body { color: {{ $reportTemplate->printPageSettings()['primary_color'] }}; font-family: {{ match($reportTemplate->printPageSettings()['font_family']) { 'times' => '"Times New Roman", serif', 'georgia' => 'Georgia, serif', default => 'Arial, Helvetica, sans-serif' } }}; font-size: {{ $reportTemplate->printPageSettings()['font_size'] }}pt; line-height: 1.2; margin: 0; box-sizing: border-box; padding: 0 {{ $reportTemplate->printPageSettings()['margin_right_mm'] }}mm 0 {{ $reportTemplate->printPageSettings()['margin_left_mm'] }}mm; width: 100%; }
        .pdf-header-offset { padding-top: {{ $reportTemplate->printPageSettings()['margin_top_mm'] }}mm; width: 100%;}
        .print-table { display: flex; flex-direction: column; width: 100%; flex-shrink: 0;}
        .print-table-row { display: grid; width: 100%;}
...
<body><div class="pdf-header-offset">@include('reports.partials.print-header')</div></body>
</html>
