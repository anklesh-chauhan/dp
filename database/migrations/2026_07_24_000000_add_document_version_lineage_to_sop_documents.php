    ...
    public function up(): void
    {
        Schema::table('sop_documents', function (Blueprint $table): void {
            ...
            $table->foreignId('supersedes_document_id')->nullable()->after('document_series_id')
                ->constrained('sop_documents')->cascadeOnUpdate()->restrictOnDelete();
            $table->text('revision_reason')->nullable()->after('version');
        });

        DB::table('sop_documents')->select('id')->orderBy('id')->eachById(function (object $document): void {
            DB::table('sop_documents')->where('id', $document->id)
                ->update(['document_series_id' => (string) Str::uuid()]);
        });

        Schema::table('sop_documents', function (Blueprint $table): void {
            $table->dropUnique('sop_documents_document_number_unique');
