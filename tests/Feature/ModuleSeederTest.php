...
it('seeds only core and DMS permissions for a DMS installation', function (): void {
    ...
    expect(Role::findByName('sop administrator', 'web')->permissions->pluck('name')->all())
        ...
        ->toBeTrue()
        ->and(Role::findByName('document controller', 'web')->hasPermissionTo('MakeEffective:ControlledDocument'))
        ->toBeTrue()
        ->and(Role::findByName('document controller', 'web')->hasPermissionTo('AssignTraining:ControlledDocument'))
        ->toBeTrue()
