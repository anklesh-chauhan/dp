...
beforeEach(function (): void {
    ...
    foreach (['Submit:SopTemplate', 'Decide:SopTemplateApproval', 'Publish:SopTemplate'] as $permission) {
        Permission::findOrCreate($permission, 'web');
    }

    $this->reviewerRole = Role::findOrCreate('qa reviewer', 'web');
    $this->approverRole = Role::findOrCreate('sop approver', 'web');
    $this->author = User::factory()->create()->givePermissionTo('Submit:SopTemplate');
    $this->reviewer = User::factory()->create();
    $this->reviewer->assignRole($this->reviewerRole);
    $this->reviewer->givePermissionTo('Decide:SopTemplateApproval');
    $this->approver = User::factory()->create();
    $this->approver->assignRole($this->approverRole);
    $this->approver->givePermissionTo('Decide:SopTemplateApproval');
    $this->publisher = User::factory()->create()->givePermissionTo('Publish:SopTemplate');

    $this->template = SopTemplate::factory()->create(['created_by' => $this->author->id]);
    $this->version = SopTemplateVersion::factory()->create([
        'sop_template_id' => $this->template->id,
...
it('prevents authors from deciding and returns rejected submissions for correction', function (): void {
    ...
    $this->author->givePermissionTo('Decide:SopTemplateApproval');
...
it('preserves submission audit events and workflow approval instances', function (): void {
    ...
    expect(SopTemplateApprovalEvent::query()->count())->toBe(1)
        ->and(SopTemplateApprovalInstance::query()->count())->toBe(2);
});
