...
final class ValidationMasterPlanEvent extends Model implements ElectronicSignatureRecord
{
    ...
    {
        self::updating(fn () => throw new LogicException('Validation master plan events are append-only.'));
        self::deleting(fn () => throw new LogicException('Validation master plan events are append-only.'));
    }
    ...
    {
        return [
            'from_status' => ValidationMasterPlanStatus::class,
            'to_status' => ValidationMasterPlanStatus::class,
            'context' => 'array',
