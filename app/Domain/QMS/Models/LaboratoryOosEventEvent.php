...
use App\Models\User;
use Database\Factories\Domain\QMS\Models\LaboratoryOosEventEventFactory;
...
final class LaboratoryOosEventEvent extends Model implements ElectronicSignatureRecord, HasSignatureContentDigest
{
    /** @use HasFactory<LaboratoryOosEventEventFactory> */
    use HasFactory;
    ...
    {
        self::updating(fn () => throw new LogicException('Laboratory OOS audit events are append-only.'));
        self::deleting(fn () => throw new LogicException('Laboratory OOS audit events are append-only.'));
    }
    ...
    {
        return [
            'from_status' => LaboratoryOosStatus::class,
            'to_status' => LaboratoryOosStatus::class,
            'context' => 'array',
            'occurred_at' => 'immutable_datetime',
        ];
    }

    /** @return BelongsTo<LaboratoryOosEvent, $this> */
    public function laboratoryOosEvent(): BelongsTo
    {
        return $this->belongsTo(LaboratoryOosEvent::class);
    }
