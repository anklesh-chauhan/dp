...
use App\Models\User;
use Database\Factories\Domain\QMS\Models\SiteMasterFileEventFactory;
...
final class SiteMasterFileEvent extends Model implements ElectronicSignatureRecord, HasSignatureContentDigest
{
    /** @use HasFactory<SiteMasterFileEventFactory> */
    use HasFactory;
    ...
    {
        self::updating(fn () => throw new LogicException('Site master file events are append-only.'));
        self::deleting(fn () => throw new LogicException('Site master file events are append-only.'));
    }
    ...
    {
        return [
            'from_status' => SiteMasterFileStatus::class,
            'to_status' => SiteMasterFileStatus::class,
            'context' => 'array',
            'occurred_at' => 'immutable_datetime',
        ];
    }

    /** @return BelongsTo<SiteMasterFile, $this> */
    public function siteMasterFile(): BelongsTo
    {
        return $this->belongsTo(SiteMasterFile::class);
    }
