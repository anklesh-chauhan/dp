...
use App\Models\User;
use Database\Factories\Domain\QMS\Models\ProductRecallEventFactory;
...
final class ProductRecallEvent extends Model implements ElectronicSignatureRecord, HasSignatureContentDigest
{
    /** @use HasFactory<ProductRecallEventFactory> */
    use HasFactory;
    ...
    {
        self::updating(fn () => throw new LogicException('Product recall events are append-only.'));
        self::deleting(fn () => throw new LogicException('Product recall events are append-only.'));
    }
    ...
    {
        return [
            'from_status' => ProductRecallStatus::class,
            'to_status' => ProductRecallStatus::class,
            'context' => 'array',
