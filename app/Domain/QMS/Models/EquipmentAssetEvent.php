...
use App\Models\User;
use Database\Factories\Domain\QMS\Models\EquipmentAssetEventFactory;
...
final class EquipmentAssetEvent extends Model implements ElectronicSignatureRecord, HasSignatureContentDigest
{
    /** @use HasFactory<EquipmentAssetEventFactory> */
    use HasFactory;
    ...
    {
        self::updating(fn () => throw new LogicException('Equipment asset events are append-only.'));
        self::deleting(fn () => throw new LogicException('Equipment asset events are append-only.'));
    }
    ...
    {
        return [
            'from_status' => EquipmentAssetStatus::class,
            'to_status' => EquipmentAssetStatus::class,
            'context' => 'array',
