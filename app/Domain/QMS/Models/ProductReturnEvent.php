...
final class ProductReturnEvent extends Model implements ElectronicSignatureRecord, HasSignatureContentDigest
{
    ...
    {
        self::updating(fn () => throw new LogicException('Product return events are append-only.'));
        self::deleting(fn () => throw new LogicException('Product return events are append-only.'));
    }
    ...
    {
        return [
            'from_status' => ProductReturnStatus::class,
            'to_status' => ProductReturnStatus::class,
            'context' => 'array',
