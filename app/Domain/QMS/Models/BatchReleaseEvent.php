...
final class BatchReleaseEvent extends Model implements ElectronicSignatureRecord
{
    ...
    protected $fillable = [
        'batch_release_id', 'event_uuid', 'from_status', 'to_status', 'actor_id', 'reason', 'context',
        'signature_hash', 'signature_ip_address', 'signature_user_agent', 'occurred_at',
    ];

    protected static function booted(): void
    {
        self::updating(fn () => throw new LogicException('Batch release events are append-only.'));
        self::deleting(fn () => throw new LogicException('Batch release events are append-only.'));
    }
    ...
    {
        return [
            'from_status' => BatchReleaseStatus::class,
            'to_status' => BatchReleaseStatus::class,
            'context' => 'array',
            'occurred_at' => 'immutable_datetime',
        ];
    }

    /** @return BelongsTo<BatchRelease, $this> */
    public function batchRelease(): BelongsTo
    {
        return $this->belongsTo(BatchRelease::class);
    }
